...
if [ "`grep "ro.build.version.release=5.1.1" $BPROP`" != "" ]; then
...
if [ "`grep "kernel.governor=conservative" $PROP`" != "" ]; then
...
elif [ "`grep "kernel.governor=alucard" $PROP`" != "" ]; then
	...
	echo "smartass2" > $GOVLITTLE
	echo "smartass2" > $GOVBIG
...
if [ "`grep "kernel.archpower=true" $PROP`" != "" ]; then
	echo "1" > $ARCH_POWER

elif [ "`grep "kernel.archpower=false" $PROP`" != "" ]; then
	echo "0" > $ARCH_POWER
else
	echo "1" > $ARCH_POWER
fi
...
if [ "`grep "kernel.gentlefairsleepers=true" $PROP`" != "" ]; then
	echo "1" > $GENTLE_FAIR_SLEEPERS

elif [ "`grep "kernel.gentlefairsleepers=false" $PROP`" != "" ]; then
